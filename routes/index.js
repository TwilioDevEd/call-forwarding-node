...
const { Senator, State, ZipCode } = require('../models');
...
router.post('/callcongress/state-lookup', (req, res) => {
  const zipDigits = req.body.Digits;
  ...
  ZipCode
    .findOne({ where: { zipcode: zipDigits } }).get('state')
    .then(state => State.findOne({ where: { name: state } })).get('id')
    .then(stateId => res.redirect(`/callcongress/call-senators/${stateId}`));
});
...
router.post('/callcongress/set-state', (req, res) => {
  ...
  if (digitsProvided === '1') {
    ...
    return State
      .findOne({ where: { name: state } }).get('id')
      .then(stateId => res.redirect(`/callcongress/call-senators/${stateId}`));
  }

  return res.redirect('/callcongress/collect-zip');
});


function callSenator(req, res) {
  State
    .findOne({ where: { id: req.params.state_id } })
    .then(state => state.getSenators()
    .then((senators) => {
      const response = new twilio.twiml.VoiceResponse();

      response.say(
        `Connecting you to ${senators[0].name}. ` +
...
function callSecondSenator(req, res) {
  Senator
    .findOne({ where: { id: req.params.senator_id } })
    .then((senator) => {
      const response = new twilio.twiml.VoiceResponse();
